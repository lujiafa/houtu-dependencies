...
**分布式锁** — 一个注解搞定并发控制：
```java
// key 支持：空值（自动类名.方法名）、参数名（"orderId"）、SpEL（"#orderId"、"#order.id"）
...
public interface UserMapper {
    int insert(@SecurityParam User user); // 入参自动加密
...
| 注解 | 所属模块 | 用途 |
|------|---------|------|
| `@Lock` | houtu-cache | 分布式锁；key 支持：空值（自动 `类名.方法名`）、参数名（`"orderId"`）、SpEL（`"#orderId"`、`"#order.id"`）；可配置 waitTime/leaseTime |
...
|:---------:|:---:|:-----------:|:------------:|:--------------------:|
| **3.5.4** | 17 |   3.5.15    | 2025.0.3 | 2025.0.0.0 |
