...
**分布式锁** — 一个注解搞定并发控制：
```java
// key 支持：空值（自动 class.method）、参数名（"orderId"）
...
| 注解 | 所属模块 | 用途 |
|------|---------|------|
| `@Lock` | houtu-cache | 分布式锁；key 支持：空值（自动 `class.method`）、参数名（`"orderId"`）；可配置 waitTime/leaseTime |
...
|:-----:|:---:|:-----------:|:------------:|:--------------------:|
| **3.5.0** | 17 | 3.5.11 | 2025.0.1 | 2023.0.1.2 |
